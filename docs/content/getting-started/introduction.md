...
simbad is a single binary. It speaks to simbad over plain HTTPS,
...
- A thin **`cmd/simbad`** hands the assembled app to `kit.Run`, which
...
```bash
simbad page <path>                  # the command
simbad serve --addr :7777           # GET /v1/page/<path>
simbad mcp                          # the page tool, over stdio
ant get simbad://page/<path>        # the URI dereference (via a host)
```
...
simbad is a read-only client over data simbad already serves
